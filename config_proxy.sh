...
if [ -f $arquivo ]; then
	echo "O arquivo $pasta já existe"  		
else
	proxy
fi
executa
...
proxy(){
	...
	echo "Acquire::http::proxy  \"http://<usuario>:<senha>@proxy.dpf.gov.br:3128/\";" >> /etc/apt/apt.conf.d/80proxy
	echo "Acquire::ftp::proxy   \"http://<usuario>:<senha>@proxy.dpf.gov.br:3128/\";" >> /etc/apt/apt.conf.d/80proxy
	echo "Acquire::https::proxy \"http://<usuario>:<senha>@proxy.dpf.gov.br:3128/\";" >> /etc/apt/apt.conf.d/80proxy
